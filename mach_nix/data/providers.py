    ...
    def __init__(self, data_dir: str, *args, **kwargs):
        ...
        if self.system == "linux":
            self.preferred_wheels = (
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-manylinux2014_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-manylinux2010_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-manylinux1_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-manylinux_2_17_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-manylinux_2_12_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-manylinux_2_5_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-linux_{self.platform}"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-any"),
            )
        elif self.system == "darwin":
            platform = "arm64" if self.platform == "aarch64" else self.platform
            self.preferred_wheels = (
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-any"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-macosx_\d*_\d*_universal"),
                re.compile(rf".*(py{maj}|cp{maj}).?.?[\.-].*({cp_abi}|abi3|none)-macosx_\d*_\d*_{platform}"),
            )
        else:
